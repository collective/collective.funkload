...
benchmark it the baseline (or some other label if baseline isn't
appropriate)::
...
Both the "--x-label" and "--y-label" options may be given
multiple times or may use `Python regular expressions <>`_ to
create an MxN matrix of differential reports.  See the
"fl-build-label-reports --help" documentation for more details.
